...
"""
School-themed 3D raycasting mini-shooter for an Informatics project.

Key differences from the original game:
- Only 2 handcrafted levels.
- No inventory, RPG stats, keys, doors, sanity, loot, or saves.
- Slower movement + rotation, with collision sliding.
- Large clear minimap.
- Gun instead of sword, rapid fire (SPACE or left mouse).
- Level 1: destroy 2 school PCs before 60s while "Prof. Mattana" chases you.
- Level 2: boss "Musci" shoots notes; beat him to win.
- Voto system instead of HP (10 → 0 = BOCCIATO!).
"""

pygame.init()

# Gestione robusta dell'audio: se non ci sono casse/cuffie collegate, il gioco va in modalità silenziosa senza crashare
HAS_SOUND = True
try:
...
MAX_DEPTH = 20
SCALE = W // NUM_RAYS
TILE = 1.0  # we treat cell size as 1 in world units

# Movement is intentionally slow/smooth
PLAYER_MOVE_SPEED = 1.5  # units per second
PLAYER_ROT_SPEED = 1.2   # radians per second (keyboard)
MOUSE_SENS = 0.0015      # much lower than before
...
GREY = (120, 120, 120)
DKGREY = (40, 40, 40)
BOSS_WALL_COL = (80, 20, 60)  # dark purple tint for the boss arena walls
...
def make_tone(freq, duration, volume=0.3, wave="sine"):
    ...
    sr = 22050
    n = int(sr * duration)
    buf = bytearray(n * 4)  # 2 bytes/sample * 2 channels, pre-allocated
    TAU = 2 * math.pi
    for i in range(n):
        t = i / sr
        if wave == "sine":
        ...
        struct.pack_into("<hh", buf, i * 4, sample, sample)  # L + R
        ...
        for y, row in enumerate(raw):
            for x, ch in enumerate(row):
                ...
                if ch == "P":
                    ...
                    self.pc_positions.append([wx, wy, False])
                elif ch == "B":
...
class Game:
    ...
    def reset(self):
        ...
        self.cutscene_messages = [
            ...
            "L'unico modo per salvarmi è mandare in crash i 2 PC della scuola",
            "Ma devo fare molta attenzione... la Prof. Mattana sta pattugliando la classe!"
        ]
        
        self.level1_end_messages = [
            "Ottimo! Entrambi i PC sono andati in crash! Il Prof. Musci non potrà assegnare compiti oggi!",
        ...
        self._graffiti = [
            (1.05, 5.5,  "Viva l'informatica!"),         
            (20.95, 3.5, "Il ciclo For non finisce mai"),
            (10.5, 9.45, "Aura di Musci +1000"),        
        ]
    ...
    def get_random_spawn_point(self):
        ...
        for y in range(self.map.h):
            for x in range(self.map.w):
                ...
                all_candidates.append((d, wx, wy))
                if d >= 6.0:
                    far_candidates.append((wx, wy))

        if far_candidates:
            return random.choice(far_candidates)
        if all_candidates:
            all_candidates.sort(key=lambda t: -t[0])
            _, wx, wy = all_candidates[0]
            return wx, wy
    ...
    def cast_rays(self, boss_mode=False):
        ...
        for ray in range(NUM_RAYS):
            ...
            if boss_mode:
                col = BOSS_WALL_COL
            shade = max(0.2, 1 - corrected / MAX_DEPTH)
            ...
            pygame.draw.rect(self.screen, (15, 10, 18), (x_start, 0, current_scale, max(0, wall_top)))
            pygame.draw.rect(self.screen, col, (x_start, wall_top, current_scale, wall_h))
            fcol = (45, 40, 35) if not boss_mode else (60, 0, 40)
            pygame.draw.rect(
                self.screen,
                fcol,
                (x_start, wall_top + wall_h, current_scale, H - wall_top - wall_h),
            )
    ...
    def draw_world_sprite(self, wx, wy, zbuf, image=None, fallback_color=(200, 60, 60),
        ...
        if image:
            ...
            pygame.draw.rect(self.screen, fallback_color, (x0, y0, sprite_w, sprite_h))
        ...
        if hp is not None and max_hp is not None and max_hp > 0 and sprite_h > 15:
            ...
            bx = screen_x - bw // 2
            by = y0 - 12
            pygame.draw.rect(self.screen, DKRED, (bx, by, bw, 6))
            pygame.draw.rect(self.screen, GREEN, (bx, by, int(bw * ratio), 6))
        ...
        if label and sprite_h > 25:
            ...
            self.screen.blit(txt, (screen_x - txt.get_width() // 2, y0 - bar_offset - txt.get_height()))
    ...
    def draw_graffiti(self, wx, wy, text, zbuf):
        ...
        if 0 <= ray_idx < NUM_RAYS and (zbuf[ray_idx] + 0.25) < dist:
            return
            
        screen_x = int((angle / FOV + 0.5) * W)
        size = max(18, min(60, int(32 / dist * 4.5)))
        try:
        ...
        t = time.time()
        r = min(255, max(0, int(230 + 25 * math.sin(t * 1.3 + wx))))
        g = min(255, max(0, int(230 + 25 * math.sin(t * 0.9 + wy))))
        b = min(255, max(0, int(40  + 60 * math.sin(t * 1.7))))
        surf = fnt.render(text, True, (r, g, b))
        shadow = fnt.render(text, True, (0, 0, 0))
        sx = screen_x - surf.get_width() // 2
        sy = HH - surf.get_height() // 2
        for ox, oy in ((-2, 2), (2, 2), (-2, -2), (2, -2)):
            self.screen.blit(shadow, (sx + ox, sy + oy))
        self.screen.blit(surf, (sx, sy))

    def draw_minimap(self):
        # MODIFICA: Cella rimpicciolita da 16 a 11 pixel per non sovrapporsi ai testi superiori
        cell_px = 11
        ...
        for y in range(self.map.h):
            for x in range(self.map.w):
                if not self.map.visited[y][x]:
                    continue
                c = self.map.grid[y][x]
                col = (60, 60, 60) if c == 0 else (150, 140, 120)
                pygame.draw.rect(surf, col, (x * cell_px, y * cell_px, cell_px, cell_px))

        if self.state in ("level1", "game_start_delay", "intro_cutscene", "end_cutscene", "transition"):
        ...
        pygame.draw.rect(self.screen, WHITE, (ox, oy, mw, mh), 1)
    ...
    def draw_hud(self):
        ...
        bw, bh = 220, 20
        x, y = 10, sh - 40
        pygame.draw.rect(self.screen, DKGREY, (x, y, bw, bh))
        ratio = self.player.hp / self.player.max_hp
        pygame.draw.rect(self.screen, GREEN if ratio > 0.3 else RED, (x, y, int(bw * ratio), bh))
        pygame.draw.rect(self.screen, WHITE, (x, y, bw, bh), 1)
        txt = self.font_md.render(f"HP: {self.player.hp}/{self.player.max_hp}", True, WHITE)
        self.screen.blit(txt, (x + 6, y - 26))

        if self.state in ("level1", "game_start_delay"):
            # timer
            tcol = RED if self.time_left <= 10 else YELLOW if self.time_left <= 25 else WHITE
            ttxt = self.font_md.render(f"Tempo: {int(self.time_left)}s", True, tcol)
            # MODIFICA: timer spostato leggermente a destra per bilanciare il testo degli obiettivi
            self.screen.blit(ttxt, (sw // 2 - ttxt.get_width() // 2 + 80, 8))

            obj = self.font_sm.render(
                f"PC in crash: {self.pcs_destroyed}/2   |   Ondate Mattana: {min(self.army_waves_spawned, self.army_waves_total)}/{self.army_waves_total}",
                True,
                WHITE,
            )
            # MODIFICA: Spostato il testo degli obiettivi a destra (+80px) per liberare la minimappa a sinistra
            self.screen.blit(obj, (sw // 2 - obj.get_width() // 2 + 80, 36))
            ...
            if close_pc and self.state == "level1":
                ...
                self.screen.blit(hint, (sw // 2 - hint.get_width() // 2, sh // 2 - 80))
            ...
            pygame.draw.rect(self.screen, PURPLE, (x, y, int(bw * ratio), bh))
            pygame.draw.rect(self.screen, WHITE, (x, y, bw, bh), 1)
            label = self.font_md.render("Prof. Musci", True, WHITE)
            self.screen.blit(label, (sw // 2 - label.get_width() // 2, y + bh + 4))

        # crosshair
        cx, cy = sw // 2, sh // 2
        pygame.draw.line(self.screen, WHITE, (cx - 8, cy), (cx + 8, cy), 1)
        pygame.draw.line(self.screen, WHITE, (cx, cy - 8), (cx, cy + 8), 1)
        ...
        if self.gun_image:
            ...
            pygame.draw.rect(self.screen, GREY, (gx, gy, gun_w, gun_h))
            pygame.draw.rect(self.screen, BLACK, (gx + 8, gy + 8, gun_w - 16, gun_h - 24))

        instr = self.font_sm.render(
            "F = Spara alla Prof. Mattana  |  F vicino al PC = Blue Screen",
            True,
            WHITE,
        )
        self.screen.blit(instr, (sw // 2 - instr.get_width() // 2, sh - 24))
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.state in ("gameover", "victory") and event.button == 1:
                        sw, sh = self.screen.get_size()
                        btn = pygame.Rect(sw // 2 - 110, sh // 2 + 70, 220, 44)
    ...
    def update_level1(self, dt):
        ...
        if self.pcs_destroyed >= 2 and self.state == "level1":
    ...
    def draw(self):
        ...
        # ── MODIFICA CRUCIALE: RISOLUZIONE OVERLAPPING MEDIANTE Z-SORTING ──
        # Raccogliamo tutti gli oggetti da renderizzare nello spazio 3D in un'unica lista
        render_queue = []

        if self.state in ("level1", "game_start_delay", "intro_cutscene", "end_cutscene"):
            # Aggiungiamo i nemici (Mattana)
            for e in self.army:
                dist = math.hypot(e.x - self.player.x, e.y - self.player.y)
                render_queue.append((dist, "enemy", e))

            # Aggiungiamo i PC della scuola
            for pc in self.map.pc_positions:
                dist = math.hypot(pc[0] - self.player.x, pc[1] - self.player.y)
                render_queue.append((dist, "pc", pc))

            # Aggiungiamo le scritte sui muri (Graffiti)
            for gx, gy, gtxt in self._graffiti:
                dist = math.hypot(gx - self.player.x, gy - self.player.y)
                render_queue.append((dist, "graffiti", (gx, gy, gtxt)))
        ...
        # Aggiungiamo i proiettili del giocatore
        for proj in self.projectiles:
            dist = math.hypot(proj.x - self.player.x, proj.y - self.player.y)
            render_queue.append((dist, "player_proj", proj))

        # Aggiungiamo i proiettili dei nemici
        for proj in self.enemy_projectiles:
            dist = math.hypot(proj.x - self.player.x, proj.y - self.player.y)
            render_queue.append((dist, "enemy_proj", proj))

        # Ordiniamo la coda in base alla distanza: dal più LONTANO al più VICINO (Z-Sorting)
        render_queue.sort(key=lambda item: item[0], reverse=True)

        # Eseguiamo il disegno seguendo il corretto ordine di profondità
        for dist, obj_type, data in render_queue:
            if obj_type == "enemy":
                ...
                if abs(angle) <= HALF_FOV:
                    ...
                    pygame.draw.circle(self.screen, YELLOW, (sx, HH), size)
            elif obj_type == "enemy_proj":
                angle = math.atan2(data.y - self.player.y, data.x - self.player.x) - self.player.angle
                if abs(angle) <= HALF_FOV:
                    sx = int((angle / FOV + 0.5) * W)
                    size = max(4, int(22 / dist))
                    pygame.draw.circle(self.screen, PURPLE, (sx, HH), size)
                    if data.label and size > 8:
                        lbl = self.font_sm.render(data.label, True, WHITE)
                        self.screen.blit(lbl, (sx - lbl.get_width() // 2, HH - size - 18))

        # overlays (Interfaccia 2D disegnata sopra il mondo 3D)
    ...
    def draw_restart_button(self, y_offset=70):
        ...
        pygame.draw.rect(self.screen, GREEN, btn, border_radius=6)
        ...
        self.screen.blit(hint, (sw // 2 - hint.get_width() // 2, btn.bottom + 10))

    def draw_game_over_screen(self):
        self.screen.fill(BLACK)
        title = self.font_lg.render("BOCCIATO!", True, RED)
        sw, sh = self.screen.get_size()
        self.screen.blit(title, (sw // 2 - title.get_width() // 2, sh // 2 - 60))
        msg = self.font_md.render("Debito Totale in Informatica", True, WHITE)
        self.screen.blit(msg, (sw // 2 - msg.get_width() // 2, sh // 2 + 10))
    ...
    def draw_cutscene_messages(self):
        ...
        pygame.draw.rect(self.screen, WHITE, (bx, by, box_w, box_h))
        pygame.draw.rect(self.screen, BLACK, (bx, by, box_w, box_h), 3)
        ...
        if self.cutscene_typing_done:
            ...
            self.screen.blit(hint, (sw // 2 - hint.get_width() // 2, by + box_h - 28))
